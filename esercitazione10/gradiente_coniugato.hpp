...
//passiamo inoltre i parametri di output per riferimento---> se fossero dichiarate nel file header nel momento in cui il 
//ciclo while termina e la funzione si chiude esse sarebbero distrutte, non potremmo stamparle nel main 
void gradiente_coniugato(const Eigen::MatrixXd& A, const Eigen::VectorXd& b, Eigen::VectorXd& x, unsigned int it_max, double res_tol, unsigned int& it_otp, double& res_norm0_otp ) 
{

	//inizializziamo  il residuo inizlae r_0 = b - A*x_0
	Eigen::VectorXd res = b - A * x;
	double res_norm_0 = res.norm();
	res_norm0_otp = res_norm_0; //passiamo il dato al main
