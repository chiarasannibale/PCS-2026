...
int main() {
	...
	for (int i=0; i < 100; i+=1){
		std::vector<int> v; 
		int dim = rand() % 1000; //in tal modo diamo limite alla dim
		...
		if (!is_sorted(v)) {
			std::cout << "Errore: l'algoritmo merge sort ha fallito al test numero" << i << "\n"; 
			return EXIT_FAILURE; 
		}
	}
		...
		//verifico che sia ordinato
		if (!is_sorted(s)) {
			std::cout << "Errore: l'algoritmo merge sort ha fallito al test delle stringhe " << "\n"; 
			return EXIT_FAILURE; 
		}

		//se non vengono segnalati problemi alsistema operativo allora i test sono avvenuti con successo
	
		std::cout << "Tutti i vettori e le stringhe testati sono stati ordinati con successo con Merge Sort"<< "\n";
		return EXIT_SUCCESS; 
}