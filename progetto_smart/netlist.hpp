...
public:
    ...
    // Costruzione: from < to. Modifica in _tipo, _valore causa shadowing
	componente(std::string _tipo, double _valore, int u, int v) {
        this->tipo   = _tipo;
		this->valore = _valore;
		n_from = std::min(u, v); 
		n_to = std::max(u, v); 
		
		//true se percorro la fem dal nodo minore al nodo maggiore
		segno_positivo=(u<v);
    }
